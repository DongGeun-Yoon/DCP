...
def save_checkpoint(epoch, epochs_since_improvement, model, optimizer, loss, is_best, dir='Normal'):
    ...
    ensure_folder(root)
    torch.save(state, os.path.join(root, 'checkpoint_{}.tar'.format(epoch+1)))
    # If this checkpoint is the best so far, store a copy so it doesn't get overwritten by a worse checkpoint
    if is_best:
        torch.save(state, os.path.join(root,'BEST_checkpoint.tar'))
...
def train_net(args):
    ...
    # load checkpoint
    if checkpoint == '':
        model = DIMModel(n_classes=1, in_channels=4, is_unpooling=True, pretrain=False)
        # init BN
    ...
    # Epochs
    for epoch in range(start_epoch, args.prune_epoch):
        ...
        # Save checkpoint
        save_checkpoint(epoch, epochs_since_improvement, model, optimizer, best_loss, is_best, dir=args.save_dir)
    # prune model
    pruning.model_prune(args)
...
def train(train_loader, teacher_model, student_model, optimizer, epoch, logger, args):
    ...
    for i, (img, alpha_label) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        loss.backward()
        update_BN(student_model, args.bns)
        # Clip gradients
